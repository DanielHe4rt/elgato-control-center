...
pub fn get_active_devices() -> Vec<ElgatoClient> {
    ...
    for device in devices.iter() {
        ...
        active_devices.append(&mut vec![client::ElgatoClient::new(device.to_string().as_str())]);
        break;
    }
    active_devices
}
...
fn get_interface() -> Result<IpAddr> {
    ...
    let default_interface = interfaces.into_iter()
    ...
    println!("Default interface: {}", interface.name);
